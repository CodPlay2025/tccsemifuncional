...
window.onload = function () {
  if (quizId) {
    localStorage.setItem('quizPersonalizadoId', quizId);
    localStorage.setItem('modoPersonalizado', 'true');
    // Não redireciona de novo para index.html
  } else {
    const fromStorage = localStorage.getItem('quizPersonalizadoId');
    if (localStorage.getItem('modoPersonalizado') === 'true' && fromStorage) {
      // já tem um quiz personalizado salvo
    } else {
      alert("Quiz inválido.");
